...
-------------------------------
The Orchestrator (runtime/orchestrator/orchestrator.py) is wired up with all three agents
...
All three agents talk to the AI models through the `ModelRouter` (core/llm/router.py),
which decides whether to use the primary server (big 7B model) or the secondary
machine (smaller, faster model).

Prompt construction
-------------------
Agents use ``PromptBuilder`` (core/prompts/builder.py) to build system and user
