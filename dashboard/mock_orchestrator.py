...
SUBSYSTEMS = [
    "ModelClient", "MemoryManager", "ToolLayer", "AgentPrompts",
    "TaskEngine", "ContextAssembler", "Orchestrator",
    "ArchitectureStateManager", "AntiStagnationSystem",
]
...
def _fake_task(status: TaskStatus = TaskStatus.ACTIVE) -> TaskInfo:
    ...
    return TaskInfo(
        ...
        started_at=now - timedelta(seconds=random.randint(0, 30))
              if status in (TaskStatus.ACTIVE, TaskStatus.COMPLETE) else None,
        completed_at=now if status == TaskStatus.COMPLETE else None,
        result_summary="Generated 3 candidate interfaces with trade-off analysis." if status == TaskStatus.COMPLETE else None,
        full_content=(
        ...
        ) if status == TaskStatus.COMPLETE else None,
    )
...
async def run_mock(tick: float = 1.5) -> None:
    ...
    while True:
        ...
        # Stagnation drift
        stagnation_score = max(0.0, min(1.0,
            stagnation_score + random.gauss(0, 0.05)))
        ...
        by_type = {
            t.value: random.randint(0, 5) for t in TaskType
        }
        ...
        patch = {
            ...
            "arch_state": {
                ...
                "last_commit_time": arch_state.last_commit_time.isoformat() if arch_state.last_commit_time else None,
