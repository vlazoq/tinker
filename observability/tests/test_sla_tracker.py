...
from observability.sla_tracker import SLATracker, SLADefinition, build_default_sla_tracker


class TestSLATracker:
    def test_define_and_report_empty(self):
        tracker = SLATracker()
        tracker.define("micro_loop", SLADefinition(p95_seconds=30.0, p99_seconds=60.0))
    ...
    def test_record_and_report_percentiles(self):
        tracker = SLATracker()
        tracker.define("micro_loop", SLADefinition(p95_seconds=10.0, p99_seconds=20.0))
    ...
    def test_breach_detected_when_over_p95(self):
        tracker = SLATracker()
        tracker.define("micro_loop", SLADefinition(p95_seconds=1.0, p99_seconds=2.0))
        # Record all values way over the threshold
        for _ in range(20):
            tracker.record("micro_loop", 10.0)
        report = tracker.report("micro_loop")
        assert report.p95_breached

    def test_no_breach_when_under_threshold(self):
        tracker = SLATracker()
        tracker.define("micro_loop", SLADefinition(p95_seconds=100.0, p99_seconds=200.0))
        for _ in range(10):
            tracker.record("micro_loop", 0.5)
        report = tracker.report("micro_loop")
        assert not report.p95_breached

    def test_all_reports_returns_all_defined(self):
        tracker = SLATracker()
        tracker.define("micro_loop", SLADefinition(p95_seconds=10.0, p99_seconds=20.0))
        tracker.define("meso_loop", SLADefinition(p95_seconds=60.0, p99_seconds=120.0))
    ...
    def test_unknown_operation_returns_none(self):
        tracker = SLATracker()
        assert tracker.report("unknown_op") is None
