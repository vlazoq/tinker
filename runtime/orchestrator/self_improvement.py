...
class SelfImprovementEngine:
    ...
    def analyze(self, snapshot: PerformanceSnapshot) -> list[SelfImprovementAction]:
        ...
        task_actions = self._analyze_failures(
            snapshot.dlq_entries, snapshot.error_counts
        )
        ...
        if len(actions) > _MAX_SELF_TASKS_PER_CYCLE:
            ...
            logger.info(
                ...
                _MAX_SELF_TASKS_PER_CYCLE, len(actions),
            )
        ...
        for subsystem, scores in subsystem_scores.items():
            ...
            if delta > 0.1:
                ...
                logger.info(
                    ...
                    subsystem, avg_first, avg_second, delta,
                )

        return actions

    def _analyze_stagnation(
        ...
        for event in events:
            ...
            self._stagnation_streak[detector] = (
                self._stagnation_streak.get(detector, 0) + 1
            )
    ...
    def _analyze_failures(
        ...
        for module, count in sorted(
            error_modules.items(), key=lambda x: x[1], reverse=True
        ):
            if count >= 2:
                actions.append(SelfImprovementAction(
        ...
        for error_class, count in sorted(
            ...
            if count >= 5:
                actions.append(SelfImprovementAction(
                    action_type="task_generation",
                    description=(
                        f"Investigate frequent '{error_class}' errors "
                        f"({count} occurrences)"
                    ),
    ...
    def apply_prompt_adjustment(
        ...
        if marker_start in current_prompt:
            ...
            current_prompt = (
                current_prompt[:start_idx] + current_prompt[end_idx:]
            ).strip()
    ...
    def apply_config_adjustment(
        ...
        if clamped != new_temp:
            logger.info(
                ...
                new_temp, clamped, self._baseline_temp, _MAX_TEMP_DELTA,
            )
        else:
            logger.info(
                "Temperature adjusted: %.3f → %.3f (delta=%.3f)",
                current_temperature, clamped, delta,
            )
