...
from prompts import (
    PromptBuilder, PromptBuilderError,
    OutputValidator, validate_output,
    SCHEMA_REGISTRY, TEMPLATE_REGISTRY, VARIANT_REGISTRY,
...
def test_all_template_keys_present():
    expected = [
        "architect.micro", "architect.meso", "architect.macro",
        "critic.micro", "critic.meso", "critic.macro",
        "researcher.micro", "researcher.meso",
        "synthesizer.meso", "synthesizer.macro",
    ]
    for key in expected:
...
def test_all_variant_keys_present():
    expected = [
        "harder_critic", "alternative_forcing", "contradiction_injection",
        "devil_advocate_critic", "socratic_architect", "paranoid_security",
        "minimum_viable_design", "scalability_stress",
    ]
    for key in expected:
...
TESTS = [
    ...
    ("Semantic: confidence too high w/ critical",  test_semantic_failure_confidence_too_high_with_critical_weakness),
    ("Semantic: accept + revision_required",       test_semantic_failure_verdict_accept_but_revision_required),
    ("Semantic: too few reasoning steps",          test_semantic_failure_architect_too_few_reasoning_steps),
    ...
    ("Variant: alt-forcing/min-viable conflict",   test_variant_alternative_forcing_minimum_viable_conflict),
]
